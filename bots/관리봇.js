        ...
        try {
            ...
            if (Array.isArray(fileHashes)) {
                for (let i = 0; i < fileHashes.length; i++) {
                    let h = truncHash(fileHashes[i]);
                    if (h && !merged.includes(h)) {
                        merged.push(h);
                    }
                }
            }
...
function onCommand(cmd) {
    try {
    let hash = truncHash(cmd.author.hash);
    ...
    switch (cmd.command) {
        ...
        case "로그": {
            ...
            try {
                ...
                // 로그 레벨 매핑 (b: 1=INFO, 2=DEBUG, 3=ERROR)
                let levelMap = { 1: "I", 2: "D", 3: "E" };

                for (let k = 0; k < recent.length; k++) {
                    let entry = recent[k];
                    // log.json 형식: {a: 메시지, b: 레벨, c: 타임스탬프}
                    let msg = entry.a || entry.content || entry.message || JSON.stringify(entry);
                    let level = levelMap[entry.b] || entry.type || "?";
                    let time = entry.c || "";

                    // 타임스탬프에서 시간 부분만 추출 (예: "2026/03/21 15:17:21" → "15:17:21")
                    if (time && time.indexOf(" ") !== -1) {
                        time = time.substring(time.indexOf(" ") + 1);
                    }

                    if (msg.length > 80) msg = msg.substring(0, 80) + "...";
                    let prefix = time ? `[${level}][${time}]` : `[${level}]`;
        ...
        Log.e("[관리봇] onCommand 오류 (!" + cmd.command + "): " + String(e));
    }
}
